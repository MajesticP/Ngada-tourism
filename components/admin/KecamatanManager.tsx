...
export default function KecamatanManager({ initialData }: { initialData: Item[] }) {
  ...
  const handleAdd = async () => {
    ...
    try {
      const res = await fetch('/api/kecamatan', {
        ...
        body: JSON.stringify({ nama_kecamatan: newNama.trim() }),
      })
      const created = await res.json()
      if (!res.ok) throw new Error(created.error)
      setData(prev => [...prev, { id: created.id_kecamatan, nama: created.nama_kecamatan, countWisata: 0 }])
      setNewNama('')
  ...
  const handleEdit = async (id: number) => {
    ...
    try {
      const res = await fetch(`/api/kecamatan/${id}`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ nama_kecamatan: editNama.trim() }),
      })
  ...
  const handleDelete = async (id: number, nama: string) => {
    try {
      const res = await fetch(`/api/kecamatan/${id}`, { method: 'DELETE' })
      ...
      <div className="lg:col-span-1">
        <div className="bg-white rounded-2xl shadow-sm border border-ngada-100 p-6">
          <h2 className="font-display text-lg text-forest-900 mb-4 flex items-center gap-2">
            <Plus size={16} className="text-ngada-500" /> Tambah Kecamatan
          </h2>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="bg-forest-800 text-white text-left">
                ...
                <th className="px-5 py-3.5 font-medium">Nama Kecamatan</th>
